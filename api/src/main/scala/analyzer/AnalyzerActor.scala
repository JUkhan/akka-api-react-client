...
case class FilteredDataHighlightText(
    datetime: LocalDateTime,
    message: String,
    highlightText: List[Position]
)
...
case class FilterResponse(
    data: List[FilteredDataHighlightText],
    dateTimeFrom: LocalDateTime,
    dateTimeUntil: LocalDateTime,
    phrase: String
)
...
trait JsonSupport extends DefaultJsonProtocol {
  implicit object LocalDateTimeJsonFormat
    ...
    override def read(json: JsValue): LocalDateTime = json match {
      ...
      case _ =>
        throw new DeserializationException("Decode local datetime failed")
    }
  }
...
class AnalyzerActor extends Actor with ActorLogging {
  ...
  override def receive: Receive = { case InitAnalyzer(fileName) =>
  ...
  def startAnalyzing(fileName: String): Receive = {
    ...
    case Filter(dtFrom, dtUntil, phrase) =>
      val data = AnalyzerActor
        .getFilteredData(fileName, dtFrom, dtUntil, phrase.toLowerCase())
        .map(it =>
          FilteredDataHighlightText(
            it.datetime,
            it.message,
            AnalyzerActor
              .findPositions(it.message.toLowerCase(), phrase.toLowerCase())
          )
        )
        .toList
      sender() ! FilterResponse(data, dtFrom, dtUntil, phrase)
    case Histogram(dtFrom, dtUntil, phrase) =>
      val filteredData = AnalyzerActor.getFilteredData(
        fileName,
        dtFrom,
        dtUntil,
        phrase.toLowerCase()
      )
      val histogramData = filteredData
        .groupBy(_.datetime)
        .mapValues(_.size)
        .map(a => HistogramData(a._1, a._2))
        .toList
...
object AnalyzerActor {
  def getFilteredData(
      ...
      dtUntil: LocalDateTime,
      phrase: String
  ): List[FilteredData] = {
    val formatter = DateTimeFormatter.ofPattern(
      "[yyyy MMM dd kk:mm:ss][yyyy MMM  d kk:mm:ss]"
    )
    val dateTime_pattern =
      "[JFMASOND][a-z]{2}\\s+\\d?\\d\\s\\d{2}:\\d{2}:\\d{2}"
    var data: List[FilteredData] = List()
    val lDate = LocalDateTime.now()
    val adtFrom =
      dtFrom.withYear(lDate.getYear).withSecond(dtFrom.getSecond - 1)
    val adtUntil =
      dtUntil.withYear(lDate.getYear).withSecond(dtUntil.getSecond + 1)
  ...
  def findPositions(str: String, subStr: String): List[Position] = {
    ...
    while (fromIndex + tempLen < strLen) {
      ...
      res = res :+ Position(index, index + tempLen - 1)
